...
import { clients, invoices, expenses, getClient } from "@/lib/mock-data";

const suggestions = ["Ferretería", "Combustible", "1048", "Constructora"];

type Result =
  | { type: "Cliente"; id: string; title: string; sub: string; href: string }
  | { type: "Factura"; id: string; title: string; sub: string; amount: number }
...
export default function BuscarPage() {
  ...
  const results = useMemo<Result[]>(() => {
    ...
    for (const inv of invoices) {
      const client = getClient(inv.clientId)?.name ?? "";
      if (
        inv.number.includes(term) ||
