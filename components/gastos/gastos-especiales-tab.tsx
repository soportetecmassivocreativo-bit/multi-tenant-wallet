...
export function GastosEspecialesTab({
  ...
  const [openIncreaseDebt, setOpenIncreaseDebt] = useState(false);
  ...
  function handleIncreaseDebt(e: React.FormEvent) {
    e.preventDefault();
    if (increaseAmount <= 0) {
      setIncreaseError("Ingresa un monto mayor a 0 para aumentar la deuda.");
      return;
    }
    setIncreaseError("");

    startTransition(async () => {
      const res = await increaseDeferredCardDebt(increaseAmount, increaseReason);
      if (res.ok) {
        setOpenIncreaseDebt(false);
        setIncreaseAmount(0);
        setIncreaseReason("");
      } else {
        setIncreaseError(res.error || "Error al aumentar la deuda.");
      }
    });
  }
  ...
  return (
    <div className="space-y-5 animate-in fade-in duration-200">
      ...
      <div className="rounded-2xl border border-line bg-soft/50 p-4 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 shadow-xs">
        ...
        <div className="flex items-center gap-2 w-full md:w-auto flex-wrap">
          <button
            type="button"
            onClick={() => {
              setOpenIncreaseDebt((v) => !v);
              setOpenNewCharge(false);
              setOpenNewAbono(false);
              setOpenEditLimit(false);
            }}
            className="flex-1 md:flex-none inline-flex items-center justify-center gap-1.5 rounded-xl border border-rose-500/30 bg-rose-500/10 px-3.5 py-2 text-xs font-bold text-rose-500 hover:bg-rose-500/20 active:scale-95 transition-all shadow-xs"
          >
            <PlusIcon className="h-3.5 w-3.5 text-rose-500" />
      ...
      {openIncreaseDebt && (
        <form
          onSubmit={handleIncreaseDebt}
          className="rounded-2xl border border-rose-500/30 bg-card p-5 shadow-lg space-y-4 animate-in fade-in duration-150"
        >
          <div className="flex items-center justify-between border-b border-line pb-2">
            <div>
              <h4 className="font-serif text-sm font-bold text-foreground flex items-center gap-2">
                ...
                Aumentar Deuda Total de la Tarjeta
              </h4>
              <p className="text-[11px] text-muted">
                Suma un nuevo monto a la deuda total pendiente de la tarjeta de José Miguel.
              </p>
            </div>
            <span className="text-xs font-bold text-foreground">
              Deuda Actual: {formatCurrency(cardLimit, "USD")}
            </span>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="block text-xs text-muted mb-1">Monto a Incrementar (USD) *</label>
              <MoneyInput
                value={increaseAmount}
                onValueChange={setIncreaseAmount}
                placeholder="0.00"
                className="w-full rounded-xl border border-line bg-soft px-3 py-2 text-xs outline-none focus:border-rose-500 font-bold"
              />
            </div>

            <div>
              <label className="block text-xs text-muted mb-1">Motivo / Concepto (Opcional)</label>
              <input
                type="text"
                placeholder="Ej. Nuevo consumo especial, ajuste de balance"
                value={increaseReason}
                onChange={(e) => setIncreaseReason(e.target.value)}
                className="w-full rounded-xl border border-line bg-soft px-3 py-2 text-xs outline-none focus:border-rose-500"
              />
            </div>
          </div>
          ...
          {increaseAmount > 0 && (
            <div className="rounded-xl bg-rose-500/10 border border-rose-500/20 p-3 flex items-center justify-between text-xs">
              <span className="text-muted">
                {formatCurrency(cardLimit, "USD")} + <strong className="text-rose-500">+{formatCurrency(increaseAmount, "USD")}</strong>
              </span>
              <span className="font-bold text-rose-500">
                Nueva Deuda Proyectada: {formatCurrency(cardLimit + increaseAmount, "USD")}
              </span>
            </div>
          )}

          {increaseError && (
            <p className="text-xs text-rose-500 font-medium">{increaseError}</p>
          )}

          <div className="flex justify-end gap-2 pt-2 border-t border-line">
            <button
              type="button"
              onClick={() => setOpenIncreaseDebt(false)}
            ...
            <button
              type="submit"
              disabled={isPending || increaseAmount <= 0}
              className="inline-flex items-center gap-1.5 rounded-xl bg-rose-500 px-5 py-2 text-xs font-bold text-white shadow-sm hover:bg-rose-600 disabled:opacity-50 transition-all"
            >
              <PlusIcon className="h-3.5 w-3.5" />
              <span>{isPending ? "Aumentando..." : "Confirmar Incremento"}</span>
            </button>
          </div>
        </form>
      )}

      {/* Modal / Formulario para Editar Límite / Deuda Base */}
      {openEditLimit && (
        <form
        ...
        >
          <div>
            <p className="text-xs font-bold text-foreground">Ajustar Deuda Base / Límite de Tarjeta</p>
            <p className="text-[11px] text-muted">Define la deuda total o límite inicial adeudado en la tarjeta de José Miguel.</p>
          </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-1">
              <button
                type="button"
                onClick={() => {
                  setOpenIncreaseDebt(true);
                  setOpenEditLimit(false);
                  setOpenNewCharge(false);
                  setOpenNewAbono(false);
                }}
                className="p-1 text-rose-500 hover:bg-rose-500/10 rounded transition-all font-bold text-xs"
              ...
              {admin && (
                <button
                  ...
                  title="Editar deuda total de la tarjeta"
                >
        ...
        <div className="rounded-2xl border border-line bg-card p-4 shadow-sm">
          ...
          <p className="text-[11px] text-hint mt-1">
            {charges.length} cargo(s) registrados
          </p>
        </div>

        {/* Tarjeta 3: Total Abonos Realizados */}
        <div className="rounded-2xl border border-line bg-card p-4 shadow-sm">
          <p className="text-xs text-muted font-medium">Abonos / Pagos Realizados</p>
          <p className="tnum mt-1 text-xl sm:text-2xl font-bold text-income">
        ...
        >
          <div className="flex items-center justify-between border-b border-line pb-2">
            ...
            <span className="text-[11px] text-muted">
              Detalle del servicio que conforma la tarjeta
            </span>
          </div>
        ...
        >
          <div className="flex items-center justify-between border-b border-line pb-2">
            <div>
              ...
              <p className="text-[11px] text-muted">
                Resta de la deuda total de la tarjeta y genera el egreso real en Gastos Generales.
              </p>
            </div>
            <span className="text-xs font-bold text-pending">
              Deuda Restante: {formatCurrency(saldoNetoPagar, "USD")}
            </span>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div className="sm:col-span-2">
              ...
              <input
                type="text"
                required
                placeholder="Ej. Pago Albañil Jose Miguel Arias, Abono a deuda tarjeta"
                value={abonoDesc}
